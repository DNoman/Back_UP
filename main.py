...
main_network = ANN()
target_network = ANN()
memory = ReplayBuffer(5000, 19, 5,discrete= True)
...
def run():
    ...
    for e in range(N_EPISODES):
        ...
        for item in range(len(observation_)):
            observation = [[item] for item in observation_]
        ...
        if score > 50 and score > maxscore:
            maxscore = score
        ...
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.title('BIỂU ĐỒ REWARD ĐẠT ĐƯỢC TRÊN MỖI EPISODE batch128')
